...
class TiktokSpider(scrapy.Spider):
    ...
    def parse(self, response):
        id = "YouTubeUserTopInfoBlockTop"
        artist = response.xpath(f"//*[@id={id}]/div[1]/h1/text()").extract()[0]
        sub = response.xpath(f"//*[@id={id}]/div[3]/span[2]/text()").extract()[0]
        like = response.xpath(f"//*[@id={id}]/div[5]/span[2]/text()").extract()[0]
        upload = response.xpath(f"//*[@id={id}]/div[2]/span[2]/text()").extract()[0]
